...
def train_models():
    ...
    # Dynamic hyperparameter selection depending on dataset size to optimize training speed and CPU usage
    batch_size = 32
    epochs = 20
    if len(X) > 100000:
        batch_size = 4096
    ...
    for label_name, y_data in y_dict.items():
        ...
        logger.info(f"Entrenando modelo para {label_name} con {num_classes} clases (epochs={epochs}, batch_size={batch_size}).")
        model.fit(X, y_train, epochs=epochs, batch_size=batch_size, verbose=0)
